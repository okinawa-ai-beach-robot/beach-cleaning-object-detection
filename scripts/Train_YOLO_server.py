...
def parse_args():
    ...
    parser.add_argument("dataset_location", nargs='?', help="path to dataset root directory", type=str, default="../Datasets/beach-cleaning-object-detection.v8-yolotrain.yolov5pytorch/")
...
def modeltrain():
    ...
    for img_width in reversed(eval_img_widths):
        ...
        if os.path.isdir(targetpath) or os.path.isdir(resultspath):
            ...
            yolov5.train.run(img=img_width, batch=16, epochs=num_epochs, data=dataset_location + "/data.yaml", cfg=yolo_path + "/models/beachbot_" + yolo_modeltype + ".yaml", weights=yolo_modeltype + ".pt", name=targetfolder + "_results", cache=True  )
        ...
        #params_export = ("--weights " + resultspath + "best.pt  --img " + str(img_heigt_export) + " " + str(img_width_export) + " --data " + dataset_location + " --include onnx" + " --opset 12 --simplify --device 0 --half").split() 
        #call(["python", yolo_path+"export.py"] + params_export)
        yolov5.export.run(weights=resultspath + "best.pt", img=str(img_heigt_export) + " " + str(img_width_export), data=dataset_location, include="onnx", opset=12, simplify=True, device=False, half=True )
