...
class ClassificationLoss:
    ...
    def calcImage(
        boxes_pred, classes_pred, confidences_pred, boxes_gt, classes_gt, iou_thresh=0.5
    ):
        # Peform performance analysis on one image
        imnr = 1
        for class_id, confidence, box in zip(
            classes_pred, confidences_pred, boxes_pred
        ):
            box_pred = box_xywh_to_xyxy(box)
            label_pred = class_id + 1
            conf_pred = confidence
            boxes_pred.append(
                metrics.BoundingBox.of_bbox(
                    imnr,
                    label_pred,
                    box_pred[0],
                    box_pred[1],
        ...
        for box, label in zip(boxes_gt, classes_gt):
            ...
            boxes_gt.append(
                metrics.BoundingBox.of_bbox(
                    imnr, label_gt, box_gt[0], box_gt[1], box_gt[2], box_gt[3], 1.0
                )
            )
    ...
    def calcDataset(dataset, model, iou_thresh=0.5, verbose=True, progress=True):
        ...
        for imnr in range(num_images):
            if progress:
                printProgressBar(
                    imnr, num_images, prefix="Progresss:", suffix="Complete", length=50
                )
